...
if (isset($_POST['register'])) {
    ...
    if (empty($username) || empty($password) || empty($referral_code)) {
        ...
        $referral_setting = $db->fetchOne("SELECT * FROM `settings` WHERE `option_name` = 'referral_code' AND `option_value` = '{$db->real_escape_string($referral_code)}' AND `table_name` = 'member'");
